...
public static class IntentStateMachinePanel
{
    ...
    public static InfoModPanel? Create(Creature owner)
    {
        ...
        {
            panel.AddLabel(Loc.Get("intent.no_metadata"), GrayMissing);
            return panel;
        }
            ...
            && !_phase1Blocklist.Contains(monsterId))
        {
            ...
            {
                initialLabels[targetId] = string.Join("、", labels) + Loc.Get("intent.initial");
            }
        }
        ...
        {
            initialLabels[startId] = Loc.Get("intent.initial");
        }
