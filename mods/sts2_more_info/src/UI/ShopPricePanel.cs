...
public static class ShopPricePanel
{
    ...
    {
        var panel = InfoModPanel.Create(Loc.Get("shop.title"), Loc.Get("shop.subtitle"));
        panel.AddSeparator();
        ...
        panel.AddRow(string.Format(Loc.Get("shop.removal"), removalCost), "", removalColor, removalColor);
        ...
        AddCell(grid, Loc.Get("shop.col_common"), GrayColor, HeaderSize);
        AddCell(grid, Loc.Get("shop.col_uncommon"), GrayColor, HeaderSize);
        AddCell(grid, Loc.Get("shop.col_rare"), GrayColor, HeaderSize);

        // Relics: 175 / 225 / 275, ±15% (v0.103.2 RelicModel.MerchantCost)
        AddCategoryRow(grid, Loc.Get("shop.relics"), GoldColor, multiplier,
            new[] { 175, 225, 275 }, 0.15f, playerGold);

        // Cards: 50 / 75 / 150, ±5%
        AddCategoryRow(grid, Loc.Get("shop.cards"), BlueColor, multiplier,
            new[] { 50, 75, 150 }, 0.05f, playerGold);

        // Potions: 50 / 75 / 100, ±5%
        AddCategoryRow(grid, Loc.Get("shop.potions"), GreenColor, multiplier,
            new[] { 50, 75, 100 }, 0.05f, playerGold);

        panel.AddSeparator();
        panel.AddLabel(Loc.Get("shop.colorless_note"), GrayColor);
    ...
    private static void AppendDiscountSummary(InfoModPanel panel, Player? player, float multiplier)
    {
        ...
        try
        {
            ...
            panel.AddLabel(Loc.Get("shop.discount_active") + " " + string.Join(", ", names), GoldColor);
        }
        catch { /* swallow */ }
    }
