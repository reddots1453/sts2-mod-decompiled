...
public static class UnknownRoomPanel
{
    ...
    public static InfoModPanel Create(UnknownMapPointOdds odds, IRunState? runState = null)
    {
        var panel = InfoModPanel.Create(
            Loc.Get("unknown.title"),
            Loc.Get("unknown.subtitle"));
        panel.AddSeparator();
        ...
        AddRow(panel, Loc.Get("room.event"),    ev,        EventColor);
        AddRow(panel, Loc.Get("room.monster"),  monster,   MonsterColor);
        if (elite > 0f)
            AddRow(panel, Loc.Get("room.elite"), elite,    EliteColor);
        AddRow(panel, Loc.Get("room.treasure"), treasure,  TreasureColor);
        AddRow(panel, Loc.Get("room.shop"),     shop,      ShopColor);
        ...
        {
            panel.AddSeparator();
            panel.AddLabel(Loc.Get("unknown.relic_modified"), GrayColor);
        }
