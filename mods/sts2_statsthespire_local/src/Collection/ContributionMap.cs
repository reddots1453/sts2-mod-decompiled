...
public class ContributionMap
{
    ...
    private static readonly AsyncLocal<List<(string sourceId, string sourceType, int amount)>> _pendingOrbValueContribsAL = new();
    ...
    private List<(string sourceId, string sourceType, int amount)> _pendingOrbValueContribs
    {
        get => _pendingOrbValueContribsAL.Value ??= new();
        set => _pendingOrbValueContribsAL.Value = value;
    }
    ...
    public IReadOnlyList<(string sourceId, string sourceType, int amount)> PendingOrbValueContribs => _pendingOrbValueContribs;
    ...
    public void MarkOrbFirstTriggerUsed() => _orbFirstTriggerUsed = true;
    public void ResetOrbFirstTrigger() => _orbFirstTriggerUsed = false;

    public void AddOrbValueContrib(string sourceId, string sourceType, int amount)
    {
        if (amount > 0)
            _pendingOrbValueContribs.Add((sourceId, sourceType, amount));
    }

    public List<(string sourceId, string sourceType, int amount)> ConsumeOrbValueContribs()
    {
        var result = _pendingOrbValueContribs;
        _pendingOrbValueContribsAL.Value = new();
        return result;
    }
    ...
    public void ClearActiveOrbContext()
    {
        ...
        _pendingOrbValueContribs.Clear();
    }
