...
public class NSaturationExampleVfx : Node
{
	...
	public override void _Ready()
	{
		...
		_tween = CreateTween();
		_tween.SetLoops();
		_tween.TweenProperty(_env, "environment:adjustment_saturation", 0f, 1.0).SetEase(Tween.EaseType.InOut).SetTrans(Tween.TransitionType.Cubic);
		_tween.TweenProperty(_env, "environment:adjustment_saturation", 1f, 1.0).SetEase(Tween.EaseType.InOut).SetTrans(Tween.TransitionType.Cubic);
	}

	public override void _Input(InputEvent inputEvent)
	{
		if (inputEvent is InputEventKey inputEventKey && inputEventKey.IsReleased() && inputEventKey.Keycode == Key.Key3)
		{
			if (_tween.IsRunning())
			{
