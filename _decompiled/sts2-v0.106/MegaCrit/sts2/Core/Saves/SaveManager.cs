...
public class SaveManager : IProfileIdProvider
{
	...
	public async Task<bool> TryFirstTimeCloudSync()
	{
		...
		List<Task> tasks = new List<Task>();
		foreach (Task item in EnumerateFirstTimeCloudSyncTasks(cloudSaveStore))
		{
			tasks.Add(item);
			if (tasks.Count >= 8)
			{
	...
	private IEnumerable<Task> EnumerateFirstTimeCloudSyncTasks(CloudSaveStore cloudStore)
	{
