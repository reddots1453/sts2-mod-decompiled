...
public class NKaiserCrabBossVfx : Node
{
	...
	{
		string text = new MegaAnimationState(animationState).GetCurrent(2)?.GetAnimation().GetName();
		if (text != "right/charged_loop" && text != "right/charge_up" && text != "right/hurt_charged")
		{
			OnChargeSteamEnd();
		}
		if (text != "right/attack_heavy")
		{
			OnRocketThrustEnd();
		}
	}
